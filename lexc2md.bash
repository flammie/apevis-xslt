...
gawk '/^LEXICON [A-Z][A-Z][^_]*$/,/^LEXICON [A-Z]*_/ {print;}' $1 |\
    ...
    sed -e 's/ )/)/' |\
    sed -e 's/^/| /' -e 's/$/ |/' > ${DICTEMP}
ALPHABET=$(sed -e 's/^..\(.\).*/\1/' < ${DICTEMP}| tr '[:upper:]' '[:lower:]' |\
    sort | uniq | egrep '[[:alpha:]]'  )
...
echo
echo "-----------------"
echo "<small>Generated with [ApeVis](https://github.com/flammie/apevis-xslt)"
echo "lexc2md 0.1</small>"
